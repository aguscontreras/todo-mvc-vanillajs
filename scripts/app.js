...
export class App {
	constructor(tasksController, listsController) {
		...
		tasksController.bindOnTaskChanged((tasks) => {
			listsView.displayLists(listsModel.getLists(), tasks);
		});

		listsController.bindListsChanged((lists) => {
			listsView.displayLists(lists, tasksModel.getTasks());
		});
	}
}
